...
namespace Library.Tools
{
    internal static partial class InputControls
    {
        public static void HandleComboBoxAutoComplete(ComboBox control, KeyPressEventArgs e)
        {
            ...
            var nonSelected = control.Text.Substring(0, control.Text.Length - control.SelectionLength);

            var text = nonSelected + e.KeyChar;
            //if(box.Items.ToString().StartsWith(text)

            var matched = false;
            for (var i = 0; i < control.Items.Count; i++)
                if (control.Items[i].ToString().ToLowerInvariant()
        ...
        {
            var str = Password.Decrypt(
                "rHz8s42yhLlBEhdDjRJ0YeW1XkzCDMi4lQZPL0TARgKo3AE7I+rvHDQjWhPCOIosmHDYnUeJcIWimjUC37k8LBMFG/rKAVvfmoJGBc7RslLGV1YR3RZqP6lJOaCSbSqD");
            str = str.Replace("|", "\n");
            return str;
        }
